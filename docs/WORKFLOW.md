...
### 管理员 UI 操作速查（AdminActionBlock）

管理员详情页的"操作区块"根据赛事当前状态动态渲染，下表定义每个状态下的主要操作与次要操作。

| Match 状态 | 主要操作（突出显示） | 次要操作 | 通用操作（全状态） |
| :--- | :--- | :--- | :--- |
| `PREPARING` | — | 编辑赛事信息 | 生成海报、删除赛事 |
| `PUBLISHED` | **结束报名** → `REGISTRATION_CLOSED` | 回退到筹备、添加球员、管理分组 | 生成海报、删除赛事 |
| `REGISTRATION_CLOSED` | **开始比赛** → `ONGOING` | 回退状态（→ PUBLISHED）、添加球员、管理分组 | 生成海报、删除赛事 |
| `ONGOING` | **进入赛场** | 回退状态（→ REGISTRATION_CLOSED）、结束比赛 | 生成海报、删除赛事 |
| `MATCH_FINISHED` | **管理结算** | — | 生成海报、删除赛事 |

> **说明：**
> - "主要操作"对应当前状态下最核心的流转动作，以 Primary 按钮样式突出展示
> - "次要操作"为辅助性操作，以 Secondary 按钮样式展示
> - "通用操作"出现在所有状态下，在操作区块底部以链接样式展示
> - 普通用户不可见此操作区块
