...
## ADR-0009 · Person Identity v1（常客 + 活动内合并 + 改名）
...
1. **流动球员**：同一人先在 A 队、后换 B 队踢球，战报按 `roster.id` 统计导致数据拆分。
2. **昵称与展示名不一致**：微信报名导入的昵称（如 emoji / 英文名）与场上希望展示的中文名不同，缺少统一改名入口。

原 `DEVELOPMENT_PLAN.md` §1.4 将「球员身份在多次活动间复用」列为 v3+ Should-Have。需求方在 MVP 验证通过后，决定将其中 **v1 子集** 提前到 Post-MVP Release R1。

### 决策

| 维度 | 选择 |
|---|---|
| **核心实体** | 新增服务端 `person`（自然人），**不是**用户账号、不是球员档案 |
| **关联** | `roster.person_id` → `person`；`game_event` 仍引用 `roster.id` |
| **统计聚合** | 活动战报（射手榜 / 助攻榜 / MVP）按 **personId** 合并；积分榜仍按 team |
| **跨活动复用** | v1 仅做「配队时从常客列表选已有 person」；**不做**跨活动历史累计报表 |
| **改名** | `PATCH /api/persons/:id {displayName}`；同步 `roster.name`；战报读 person 当前名（非事件快照） |
| **录分 UI** | **不改** `GoalPickPanel`；合并发生在 `report.service` |
| **Person 作用域** | 单 SQLite 实例全局共享（玩具单俱乐部假设；零登录） |
| **微信导入** | v1 仍每条新建 person；不自动匹配已有常客（防误合并） |
| **明确不做** | person 合并去重、person 删除、档案扩展字段、跨活动统计 |

### 与 Out-of-Scope 的边界

- §1.3「球员档案、身价、能力面板」：**不引入**。`person` 仅有 `displayName` + 时间戳，不是 FM 式档案。
- §1.3「用户注册」：**不引入**。person 由管理员维护，无登录绑定。

### 替代方案对比

| 方案 | 弃用理由 |
|---|---|
| 按 `roster.name` group by | 同名不同人误合并；无法表达「同一人多 roster」 |
| 仅 localStorage 常客、无服务端 person | 改名无法跨设备；与 SQLite 权威数据不一致 |
| 活动级 `event_person` | 无法跨活动复用常客；每次活动重录 |
| 录分时选「自然人」而非 roster | 伤害北极星（多一步）；且需知道当前代表哪条 roster |
...
- 常客跨活动复用，减少重复录入
- 改名统一入口，微信昵称可事后修正
- `game_event` 不变，迁移风险可控（roster 1:1 回填 person）

**负面**：

- 表数 +1、API +3、配队 UI 变复杂；需严格守北极星（常客 ≤3 点击）
- 改名后旧战报若重新生成会显示新名（v1 接受；见 PLAN R7）
- 微信导入可能产生重复 person，需 v2 合并工具

### 关联文档

- `DEVELOPMENT_PLAN.md` §1.4、§2.6、§4.3、D16
- `docs/ARCHITECTURE_V2.md` §3（ER + 迁移）、§4.3.1（API）、§7.2–§7.3（战报）、§8.7（前端）
