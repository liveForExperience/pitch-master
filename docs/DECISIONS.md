...
1. **触发方式**：是 GitHub Actions 推、ECS pull、webhook 触发，还是容器编排？v1 用过 webhook（adnanh/webhook + GitHub Webhook），密钥用了默认占位符 `your-secret-token` 是安全隐患；同时构建仍在 ECS 上做会跟应用抢内存。
2. **Caddy vs Nginx**：ARCHITECTURE_V2 §10 写的是 Caddy（自动 HTTPS），但 ECS 上 v1 已下线后 Nginx 还在跑，且当前域名/HTTPS 不是 Phase 0 必需。
...
| **Web 服务器** | **保留 Nginx**（v1 已在跑，无需再装 Caddy），路由：`/api/* → :3000`，其它 → `/opt/pitchmaster-v2/current/web/dist`，SPA fallback |
...
| 复用 v1 的 webhook 模式 | 密钥管理散乱，已有 v1 默认占位密钥的前车之鉴 |
| Watchtower / 镜像编排 | 引入容器与镜像仓库，违背 v2 minimalist 原则 |
...
- Release 留 5 份，回滚秒级
- 凭证集中在 GitHub Secrets，比 v1 hardcoded 在 hooks.json 安全得多
