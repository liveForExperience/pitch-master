...
- **多人录入渐进方案（Phase 5 · 已立项草案 2026-06-19）**：S2 录入权租约交接 → S3 多写者事件日志；详见 **§2.6** 与 **§4.3**
...
### 2.6 Phase 5 · 多人录入（S2 → S3 渐进 · 草案 v0.1）

**触发条件**：Phase 2 Gate 通过 + 需求方签署 **§4.3**（2026-06-19 已起草，待签）。

**北极星约束**：S2/S3 不得拖慢「开页面 → 记一个进球 ≤ 8 秒」；冲突提示必须非阻塞（后台 reconcile）。

**总周期**：约 3–4 周业余时间（S2 约 12–16h，S3 约 20–28h）。S3 **依赖** S2 的 `deviceId`、租约基础设施与测试夹具。

#### Phase 5A · S2 录入权交接（Editor Lease）

**目标**：同一场比赛任意时刻**仅一台设备**可写；支持显式「交给另一台手机」而**不必**轮换 `adminToken`（保留 PIN 恢复作为换机兜底）。


| 任务                    | 产出物                                                                                |
| --------------------- | ---------------------------------------------------------------------------------- |
| T5A.1 DDL + migration | `game` 表增 `editor_device_id`、`editor_lease_expires_at`；见 `ARCHITECTURE_V2.md` §6.6 |
| T5A.2 租约 service      | `editor-lease.service.ts`：claim / renew / release / force-claim                    |
| T5A.3 写接口门禁           | 场次写操作（start/pause/resume/finish/events/batch）无有效租约 → `409 editor_lease_required`   |
| T5A.4 API 路由          | `POST/DELETE /api/games/:id/editor`；`GET` 详情含 `editor` 字段                          |
| T5A.5 SSE             | 广播 `editor_changed`                                                                |
| T5A.6 前端 deviceId     | `web/src/lib/device-id.ts` 持久化 `pm:deviceId`                                       |
| T5A.7 录入页集成           | `GameRecordPage` 进入 claim、30s 续租、离开 release；无锁只读 +「申请录入权」                          |
| T5A.8 观战提示            | `GameDetailPage` 显示当前录入设备（尾 4 位）                                                   |
| T5A.9 测试              | 租约过期、抢锁、同 device 多 Tab 续租、写接口 409                                                  |


**Phase 5A Gate**：

- ✅ A 持有租约时 B 记球 → B 收到明确提示，服务端无新事件
- ✅ A「释放录入权」→ B 一键 claim 后可记球，全程无需 PIN
- ✅ A 断网/杀进程 90s 后租约过期，B 可 claim
- ✅ PIN 恢复仍可用（轮换 token），与租约互不破坏
- ✅ 记一球仍 ≤ 8 秒（含首次 claim）

#### Phase 5B · S3 多写者协同（Multi-Writer Event Log）

**目标**：**进球/撤销**允许多设备同时写；**计时**仍单持有者（复用 S2 租约，仅 timer 类操作校验）。


| 任务                  | 产出物                                                                       |
| ------------------- | ------------------------------------------------------------------------- |
| T5B.1 排序权威化         | 展示与 replay 主序 `serverTs`，`clientTs` 仅 tie-break；更新 batch sort             |
| T5B.2 乐观并发          | `game.version` 整数；timer 变更 `WHERE version=?`，失败 409 + 最新 state            |
| T5B.3 写门禁拆分         | GOAL/OWN_GOAL/UNDO **不**要求 editor lease；start/pause/resume/finish **仍要求** |
| T5B.4 客户端 reconcile | `mergeGameWithOutbox` 升级为 server-first；SSE 带 `eventVersion` 或增量           |
| T5B.5 UNDO 规则       | 仅允许撤销已确认（有 `serverTs`）事件；pending 本地球仅本机可撤                                 |
| T5B.6 冲突 UI         | 409 静默 `GET /api/games/:id` 重绘，不打断点击流                                     |
| T5B.7 双写集成测试        | 两设备交错记球 + 各撤一球 + 弱网 batch flush                                           |
| T5B.8 文档            | 更新 ARCH §6.7；`deriveScore` 变更有单测                                          |


**Phase 5B Gate**：

- ✅ 两手机同时记 5 球，比分正确，时间线按 `serverTs` 稳定
- ✅ 设备 A 离线记 2 球，设备 B 在线记 3 球，双端联网后一致
- ✅ 交叉 UNDO 不丢球、不产生幽灵分
- ✅ 仅 timer 持有者可 pause；非持有者操作 409
- ✅ 飞行模式剧本在双设备场景仍通过（幂等 + 顺序）

**明确不做（Phase 5 范围外）**：

- 用户注册 / 角色权限矩阵（见 §1.3）
- CRDT / Yjs 实时协同编辑
- 活动配置页（roster）多人同时改——仍任意 adminToken 可写；冲突靠「别两人同时配队」运营约束，v4 再议
...
| ID  | 风险                            | 等级  | 缓解                                                                                             |
| --- | ----------------------------- | --- | ---------------------------------------------------------------------------------------------- |
| R1  | 离线队列冲突合并复杂度高（多设备同时录入同一场比赛）    | 高   | **Phase 5A**：场次级 editor lease 单写者；**Phase 5B**：server-first reconcile + UNDO 约束；详见 §2.6 / §4.3 |
...
| M1     | Phase 5 多人录入（S2→S3）签署        | ~~需求方~~ | **已决 2026-06-19：§4.3 已签署，Phase 5A 开工** |
| M2     | S3 是否允许「强制抢 timer 租约」需二次确认   | ~~需求方~~ | **已决 2026-06-19：允许，前端 confirm 弹窗** |


### 4.3 方案 · 多人录入渐进（Phase 5 · S2→S3 · 草案 2026-06-19）

**背景**：球场常见「记完上半场把手机交给另一个人」；v2 用 PIN 轮换 token 能换机，但体验重、且无法防止两人误同时录入。需求方确认路径：**先 S2（交接）再 S3（真协同）**。
...
| ------ | ------------------------ | ------ |
| S1     | 多人观战（SSE 只读）             | 已有     |
| **S2** | 录入权交接：任意时刻单写者，可显式交给另一台设备 | **5A** |
| **S3** | 多人同时记进球/撤销；计时仍单人控        | **5B** |
...
| 规则       | 值                                         |
| -------- | ----------------------------------------- |
| 租约粒度     | **场次** `game_id`（非活动级）                    |
| 持有者标识    | `deviceId`（客户端 `localStorage` UUID，非用户账号） |
| 默认 TTL   | 90s；录入页每 30s 自动 renew                     |
| 过期       | 无续租则自动释放；他人可 claim                        |
| 抢锁       | `force=true` 可抢（需 adminToken + 前端二次确认）    |
| 同设备多 Tab | 共享 `deviceId`，视为同一持有者                     |
| 写门禁      | 无租约 → `409` code=`editor_lease_required`  |
| 只读       | 无 adminToken 或无租约均可观战（SSE / 详情页）          |


**与 PIN 恢复的关系**：

- PIN 恢复：**换机 / 丢 token** 兜底，仍轮换 `adminToken`
- Editor lease：**同 token 多设备** 时的录入权调度，不轮换 token
...
| 规则        | 说明                                            |
| --------- | --------------------------------------------- |
| 进球/撤销     | 多设备可同时 POST；靠 append-only `game_event` + 幂等收敛 |
| 权威顺序      | `serverTs ASC`，同毫秒 `clientEventId` 字典序        |
| 计时        | 仍走 S2 租约；仅 holder 可 start/pause/resume/finish |
| 并发写 timer | `game.version` 乐观锁；失败返回最新 `GameDetail`        |
| UNDO      | 目标必须有服务端 `id`；跨设备不得撤他人 pending outbox         |
| 不用 CRDT   | 足球记分可交换性强，事件溯源足够                              |
...
1. `backend/src/db/schema.ts` + migration
2. `backend/src/services/editor-lease.service.ts` + test
3. `backend/src/lib/editor-guard.ts` 挂到 `games.ts` 写路由
4. `web/src/lib/device-id.ts`
5. `web/src/lib/use-editor-lease.ts`
6. `web/src/pages/GameRecordPage.tsx` + `GameDetailPage.tsx`

**S3 在 S2 之上**：

1. `backend/src/services/outbox.service.ts` 排序调整
2. `game` 表 `version` + `game-ops.service.ts` 乐观锁
3. `web/src/lib/outbox/merge-game.ts` → `reconcile-game.ts`
4. `backend/tests/multi-writer.integration.test.ts`

#### 验收剧本（人工 · 内测追加）

1. **S2**：A 记 2 球 → 释放 → B claim → 记 1 球 → 观战端 SSE ≤ 2s
2. **S2**：A 持锁杀进程 → 90s 后 B 抢锁成功
3. **S3**：A/B 各快速点 3 球 → 比分 6，流水顺序稳定
4. **S3**：A 飞行模式记 2 球，B 在线记 3 球 → 联网后服务端 5 球
...
### 2026-06-19 · Phase 5 多人录入计划（S2→S3 渐进）

需求方确认路径：**先 S2（录入权交接）再 S3（真协同录入）**。已登记文档，**代码未开工**。


| 项                     | 状态          | 说明                                    |
| --------------------- | ----------- | ------------------------------------- |
| §2.6 Phase 5A/5B 任务分解 | ✔           | Editor Lease → Multi-Writer Event Log |
| §4.3 方案细则             | ✔           | 租约规则、UNDO 约束、验收剧本                     |
| ARCH §6.6–6.7         | ✔           | DDL / API / SSE / reconcile           |
| ADR-0008              | ✅ Accepted | 需求方 2026-06-19 签署 §4.3                |
| Phase 5A 代码           | ✅ 已交付    | editor lease 全栈 + 单测；待 Phase 5A Gate 人工验收 |
| Phase 5B 代码           | ✅ 已交付    | multi-writer + version 乐观锁 + reconcile；待 S3 验收 |

**2026-06-19 · Phase 5A/5B 多人录入（S2→S3）**：

| 项 | 状态 |
|---|---|
| T5A editor lease | ✅ |
| T5B multi-writer + `game.version` | ✅ |
| 后端单测 71 + 前端 77 | ✅ |
| ECS 部署 | 🚧 push main 触发 |
| 待决 M2                 | ✅ 已决      | 允许 force 抢 timer（二次确认弹窗）                    |
